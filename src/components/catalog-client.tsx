  ...
  return (
    <section className="min-h-0 flex-1 overflow-hidden bg-white px-[60px] py-4" aria-labelledby="browse-heading">
      <div className="flex h-full min-h-0 w-full flex-col">
        ...
        <div className="mt-3 flex shrink-0 gap-2 overflow-hidden pb-1" role="tablist" aria-label="Meal categories">
          {categories.map((item) => (
            <button
              key={item}
              type="button"
        ...
        <div className="mt-3 grid min-h-0 flex-1 grid-cols-6 gap-3 overflow-hidden">
          {visible.map((product) => (
