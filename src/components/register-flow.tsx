...
export function RegisterFlow() {
  ...
  return (
    <div className="p-8">
      ...
      {step === 1 && <form key="step-1" className="auth-card-enter mx-auto mt-7 grid max-w-xl gap-5" onSubmit={(event) => { event.preventDefault(); setStep(2); }}>
        <label className="grid gap-2 text-sm font-semibold">Full Name<input required value={data.name} onChange={(event) => update("name", event.target.value)} autoComplete="name" className="form-field h-12 rounded-lg px-4 font-normal" /></label>
        <label className="grid gap-2 text-sm font-semibold">Email Address<input required type="email" value={data.email} onChange={(event) => update("email", event.target.value)} autoComplete="email" className="form-field h-12 rounded-lg px-4 font-normal" /></label>
        <label className="grid gap-2 text-sm font-semibold">Mobile Number<input required type="tel" value={data.phone} onChange={(event) => update("phone", event.target.value)} autoComplete="tel" className="form-field h-12 rounded-lg px-4 font-normal" /></label>
        <label className="grid gap-2 text-sm font-semibold">Password<span className="relative"><input required type={showPassword ? "text" : "password"} placeholder="Create a strong password" autoComplete="new-password" className="form-field h-12 w-full rounded-lg px-4 pr-12 font-normal" /><button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute right-1 top-0 grid size-12 place-items-center rounded-lg text-muted hover:bg-brand-tint hover:text-brand" aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? <EyeOff className="size-5" /> : <Eye className="size-5" />}</button></span></label>
        <label className="grid gap-2 text-sm font-semibold">Confirm Password<input required type="password" placeholder="Re-enter password" autoComplete="new-password" className="form-field h-12 rounded-lg px-4 font-normal" /></label>
        <label className="flex items-start gap-3 rounded-lg bg-background p-3 text-xs text-copy"><input required type="checkbox" className="mt-0.5 size-4 accent-brand" />I agree to the Terms &amp; Conditions and Privacy Policy.</label>
